...
setSecondLine = ->
	...
	for letter, i in newSecondText
		...
		Utils.delay i * TYPE_DELAY, ->
			if lastInCycle
				changeText2._element.childNodes[1].childNodes[0].removeChild(cursor)
...
playNext = ->
	...
	if CYCLE_VIDEOS
		...
		if i < (videos.length - 1)
			...
			lastInCycle = true
			Events.wrap(video1.player).off "ended", playNext
			Events.wrap(video2.player).off "ended", playNext
...
Events.wrap(video1.player).on "ended", playNext
Events.wrap(video2.player).on "ended", playNext

Events.wrap(video1.player).on "canplaythrough", loadNext
Events.wrap(video2.player).on "canplaythrough", loadNext
