...
# Offers Scroll
class OffersScroll extends ScrollComponent
	constructor: (options = {}) ->
...
offersButton.onTap ->
	offersScroll.scrollToLayer(offerButtons[0])
...
coachingButton.onTap ->
	offersScroll.scrollToLayer(coachingButtons[0])
	
#-  Pages

offersScroll = new OffersScroll
	parent: view
	y: 494
	icon: 'cards'
...
for button, i in [
	...
	offerButtons[i] = new OfferButton
		parent: offersScroll.content
		x: 104 * i
		icon: button.icon
		title: button.title
		value: if SHOW_PROGRESS then button.value else null
		
separator = new cs.Icon
	type: 'notification'
	parent: offersScroll.content
	x: _.last(offerButtons).maxX + 24
...
for button, i in [
	...
	coachingButtons[i] = new OfferButton
		parent: offersScroll.content
		x: separator.maxX + 24 + (104 * i)
		icon: button.icon
		title: button.title
		value: if SHOW_PROGRESS then button.value else null

offersScroll.onMove ->
	print offersScroll.scrollX
	print separator.x + separator.width / 2 - Screen.width
	if offersScroll.scrollX > separator.x + separator.width / 2
		offersButton.isOn = false
		coachingButton.isOn = true
	else
		offersButton.isOn = true
