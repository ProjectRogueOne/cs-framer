...
class exports.CommentsBox extends Layer
	constructor: (options = {}) ->
		...
		@_vote = database.user.getVote(@content.uid)
	...
	showVotes: ->
		vote = database.user.getVote(@content.uid)
	...
	@define 'vote',
		...
		set: (vote) ->
			...
			currentVote = database.user.getVote(@content.uid)
			...
			database.user.setVote(@content.uid, currentVote)
