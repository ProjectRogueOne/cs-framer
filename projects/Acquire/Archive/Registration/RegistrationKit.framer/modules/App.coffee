...
class exports.App extends FlowComponent
	constructor: (options = {}) ->
		...
		throttledSetAppSize = Utils.throttle .05, @_setAppSize

		Framer.Device.on "change:deviceType", throttledSetAppSize
		Events.wrap(window).addEventListener "resize", throttledSetAppSize
	...
	changeViews: (view) =>
		...
		h = @height - (@navigation?.maxY ? @header?.maxY ? 0) - (@footer?.height ? 0)
	...
	@define "type",
		...
		set: (string) ->
			...
			if not _.includes(['mobile', 'safari', 'ios', 'android', 'none'], string) 
			...
			if @header?
				...
				throttledSetYPositions = Utils.throttle .05, @_updateYPositions
				@header?.on "change:height", throttledSetYPositions
