...
class DocUploadView extends RegistrationView
	...
	showDocumentUpload: =>
		if @documentSelectField.text is @documentSelectField.options[0]
			...
			@advice.opacity = 0
			@advice.illo.image = illos_passport.image
		...
		switch @documentSelectField.text
			when @documentSelectField.options[1]
				@advice.illo.image = illos_passport.image
				@advice.illo.text = 'For a passport, upload a picture of the photo page, with all four corners visible. Your document needs to be in-date with all details visible (nothing covered).'
			when @documentSelectField.options[2]
				@advice.illo.image = illos_license.image
				@advice.illo.text = 'Upload a clear picture of the front of your driving license with all four corners showing. Your licence needs to be in-date with all details visible (nothing covered). '
