...
class TimeLineComponent extends Layer
	constructor: (options = {}) ->
		...
		for layer in [@timeLineScroll, ]
		
		@timeLineScroll.contentInset =
			left: 8
			right: 8
			
		@timeLineScroll.content.draggable.momentumOptions =
			friction: 5
			tolerance: 2.5
		...
		@tileScroll.content.draggable.momentumOptions =
			friction: 5
			tolerance: 2.5
		...
		for year, months of years
			for alerts, month in months
				...
				# create a month container and add it to the scroll
		...
		@onScrollAnimationDidEnd =>
			# if we're sort of close to a content layer, snap to it
			if Math.abs(@closestContentLayer().x - @scrollX) < 100
