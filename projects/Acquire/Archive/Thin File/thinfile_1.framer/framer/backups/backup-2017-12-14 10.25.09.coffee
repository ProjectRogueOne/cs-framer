...
class DocUploadView extends RegistrationView
	constructor: (options = {}) ->
		...
		isDocResubmit = ->
			if DOC_RESUBMIT
				return {text: "Sorry, but we couldn’t validate your identity from the photos you sent last time. You’ll need to upload your photos again.", color: "#daad67"}
			
			return {text: "Next we'll need a photo of your ID and a selfie with your ID to verify that it’s yours.", color: "#5b636e"}
		
		@puller = new cs.Puller
			parent: pullerFlexrow.content
			text: isDocResubmit().text
			backgroundColor: isDocResubmit().color
