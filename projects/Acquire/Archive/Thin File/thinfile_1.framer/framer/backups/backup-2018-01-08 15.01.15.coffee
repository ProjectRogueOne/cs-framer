...
class UploadComponent extends Layer
	constructor: (options = {}) ->
		...
		@valid1Icon = new cs.Icon
			name: 'Validation 1 Icon'
			parent: @
			icon: 'cs-check'
			color: '#81af51'
			y: @bodyLayer.maxY
		...
		@valid1Body = new cs.Text
			name: 'Validation 1 Body'
			parent: @
			y: @bodyLayer.maxY
			x: 20
			text: ' '
		...
		switch value
			when 'passport'
				_.assign @,
					...
					body: 'Please make sure we can see your passport’s entire <b>photo page</b> with all information visible.'
				
				@valid1Icon.y = @bodyLayer.maxY + 8
				_.assign @valid1Body, 
					y: @bodyLayer.maxY + 8
					text: 'Are all four corners visible?'
					
				@valid2Icon.y = @valid1Body.maxY + 2
				_.assign @valid2Body, 
					y: @valid1Body.maxY + 2
			...
			when 'licence'
				_.assign @,
					...
					photo: photo_id
					body: 'Please make sure we can see the entire front side of your driving licence.'
			...
			when 'selfie'
				_.assign @,
					...
					photo: selfie_id
					body: 'For your selfie, please make sure we can see your face and your entire ID in the same photo.'
				
				@valid1Icon.y = @bodyLayer.maxY + 8
				_.assign @valid1Body, 
					y: @bodyLayer.maxY + 8
					text: 'Is the entire ID in the photo?'
					
				@valid2Icon.y = @valid1Body.maxY + 2
				_.assign @valid2Body, 
					y: @valid1Body.maxY + 2
					text: 'Is your whole face in the photo?'
...
class DocUploadView extends RegistrationView
	constructor: (options = {}) ->
		...
		if DOC_RESUBMIT
			noto = new cs.Puller
				parent: pullerFlexrow.content
				text: "Sorry, but we couldn’t validate your identity from the photos you sent last time. You’ll need to upload your photos again. Dismiss"
				fill: "#daad67"
				arrow: false
				
			_.assign noto.textLayer,
				x: 48
				width: @width - 64
...
# 			text: "Thanks to identity theives and internet jerks, we need to be sure that you're really you before we can get your credit report. We'd usually just ask about your financial history, but we haven't found info enough to safely do that. These photos are our plan B."
		...
		@contactUs = new cs.Text
			name: 'Contact us'
			parent: bodyFlexrow.content
			y: @nextButton.maxY
			text: 'Having trouble? Contact customer support.'
			width: @width
			textAlign: 'center'
...
# 			
		...
		Utils.delay 0, => 
			...
			@contactUs.y = @nextButton.maxY + 16
