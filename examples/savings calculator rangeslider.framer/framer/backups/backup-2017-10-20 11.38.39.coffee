...
savingsView.build ->
	...
	yourScoreSlider = new RangeSliderComponent
		parent: yourScore
		x: 32, y: yourCurrentScore.maxY + 16
		width: @width - 64
		min: 0
		max: 700
		minValue: 300
	...
	setValues = ->
		...
		yourCurrentScore.template = yourScoreSlider.minValue
		
		yourGoalScore.midX = yourScoreSlider.maxKnob.midX + 32
		yourGoalScore.template = yourScoreSlider.maxValue
		
		avgCurrent.template = (60 - (yourGoalScore.minValue / 13)).toFixed(1)
		avgGoal.template = (60 - (yourGoalScore.maxValue / 13)).toFixed(1)
		
		cCost = (2000 - (yourGoalScore.minValue * 1.42))
		gCost = (2000 - (yourGoalScore.maxValue * 1.42))
	...
	yourScoreSlider.on "change:value", setValues
