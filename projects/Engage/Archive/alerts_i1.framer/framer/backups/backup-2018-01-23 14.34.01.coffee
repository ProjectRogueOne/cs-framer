...
class TimeLineComponent extends PageComponent
	constructor: (options = {}) ->
		...
		last =_.last(@alerts).date.getYear()
			year: _.last(@alerts).date.getYear()
		
		for year, months of years
			for alerts, month in months
			
				if year is _.last(@alerts).date.getYear() and
				month > _.last(@alerts).date.getMonth()
					continue
				
				if year is @alerts[0].date.getYear() and
				...
				monthPage = new MonthContainer
					backgroundColor: null
					hueRotate: _.random(-40, 40)
					alerts: alerts
					year: year
					month: month
			
				@addPage(monthPage, 'right')
		
		
		
		@contentInset =
			left: 8
			right: _.clamp(@width - monthPage.width, 0, Infinity)
		
		@snapToPage(monthPage, false)
...
# 			)
# 			
# 		@height = _.last(@alertTiles).maxY + 32
