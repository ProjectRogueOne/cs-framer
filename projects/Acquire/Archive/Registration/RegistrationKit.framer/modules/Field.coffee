...
class exports.Field extends Layer
	constructor: (options = {}) ->
		...
		unless @_size is 'full'
			if options.width or options.height
				@inputLayer.width = options.width ? 128
				@inputLayer.height = options.height ? 50
			else
		...
		Utils.insertCSS """
			...
			input##{@name}::placeholder {
				-webkit-text-fill-color: #{options.placeholderColor ? options.color};
			}
	...
	isEmail: => 
		matches = @value.match(/^([\w.-]+)@([\w.-]+)\.([a-zA-Z.]{2,6})$/i)
		return matches?
