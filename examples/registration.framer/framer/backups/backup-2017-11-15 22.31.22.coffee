...
class DobView extends RegistrationView
	constructor: (options = {}) ->
		...
		@puller = new cs.Puller
			parent: @content
			y: -16
			text: "We'll also need your date of birth."
			arrow: true
	...
	stateDay: =>
		return if !_.every(@required, 'value')
		date = new Date(@year.value, @month.value, @day.value)
		@puller.text = @_startText + " Hey, you were born on a #{date.toLocaleString([], {'weekday': 'long'})}. Nice!"
