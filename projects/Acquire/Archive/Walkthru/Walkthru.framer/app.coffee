	...
	setCTAs: (array = []) =>
		...
		for cta, i in array
			@ctas[i] = new cs.Button
				parent: @
				maxY: Screen.height - 24 - (app.header?.height ? 0) - (app.footer?.height ? 0) - (i * 64)
				width: @width - 32
				x: 16
				fill: if cta.primary then 'white' else 'none'
				border: 'white'
				color: if cta.primary then 'main' else 'white'
				text: cta.text
...
nextView = -> 
	...
	views[v].once Events.AnimationEnd, ->
		v++
