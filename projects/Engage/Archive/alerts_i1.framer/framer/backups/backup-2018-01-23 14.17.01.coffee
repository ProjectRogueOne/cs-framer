	...
	constructor: (options = {}) ->
		...
		for year, months of years
			for alerts, month in months
				...
				if year >= 118 and month > _.last(@alerts).date.getMonth() or month < @alerts[0].date.getMonth()
					continue
				
				# month container
				monthPage = new MonthContainer
					backgroundColor: '#67bbd8'
					hueRotate: _.random(-40, 40)
					brightness: _.random(80, 100)
					alerts: alerts
					year: year
					month: month
