...
class PersonChoiceScroll extends ChoiceScroll
	constructor: (options = {}) ->
		...
		@yesCallback = options.callback ?= -> null
		@noCallback = options.callback ?= -> null
		...
		doneContainer.onTap => @hideButtons(false)
		noThanksContainer.onTap => @hideButtons(true)
	...
	hideButtons: (sendInvites) =>
		...
		if sendInvites
			Utils.delay 1, => @yesCallback()
		else
			Utils.delay 1, => @noCallback()
...
greeting = ->
	...
	mainChat.addChat new PersonChoiceScroll
		chatView: mainChat
		yesCallback: -> confirmPeople(true)
		noCallback: -> confirmPeople(false)
	
# 	Utils.delay 2, -> askWhat()
