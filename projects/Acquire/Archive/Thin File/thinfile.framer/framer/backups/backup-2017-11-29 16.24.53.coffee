...
class DocUploadView extends RegistrationView
	constructor: (options = {}) ->
		...
		@advice = new Advice
			parent: bodyFlexrow.content
			y: @photoUploadField.maxY + 24
			image: illos_passport.image
		
		# document notes
		
		@documentNotes = new cs.Text
			parent: bodyFlexrow.content
			y: @photoUploadField.maxY + 32
			x: 16
			width: @width - 32
		...
		@documentNotes.opacity = 0
		
		@nextButton = new cs.Button
			parent: bodyFlexrow.content
			y: @documentNotes.maxY + 45
			x: 16
			text: 'Next'
			fill: 'dcaa74'
			type: 'body'
...
class SelfieUploadView extends RegistrationView
	constructor: (options = {}) ->
		...
		@documentNotes = new cs.Text
			...
			x: 16
			width: @width - 32
			text: 'For your selfie, be sure that your face and your ID are both in the picture. Sorry if this is weird. Big smiles!'
