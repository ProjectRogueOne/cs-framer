...
class RegistrationView extends cs.View
	constructor: (options = {}) ->
		...
		@continueButton.onTap => @first = false
...
# Intro View
class IntroView extends RegistrationView
	constructor: (options = {}) ->
		@__constructor = true
		...
		@addToStack new cs.Text
			type: 'body1'
			x: Align.center
			text: "To get your free credit report, we first have to ask Equifax to release it to us. The robots who handle this part are very careful about releasing information, so we'll need to get your details right."
		
		@addToStack new cs.Text
			type: 'body1'
			text: "Let's start with your e-mail."
		...
		@continueButton.action = =>
			...
			if ALWAYS_RECOGNISE_USER or @emailField.value is 'frank@clearscore.com'
				...
				app.showNext(introViewPassword)
...
# Intro View (Password)
class IntroViewPassword extends RegistrationView
	constructor: (options = {}) ->
		@__constructor = true
...
class HomeView extends RegistrationView
	...
	setValue: (index, value, animate = true, noEdit = false) ->
		
		@rows[index].value = value
		@rows[index].showValue(noEdit)
		@current = index + 1
...
class NameView extends RegistrationView
	constructor: (options = {}) ->
		...
		@addToStack middleName = new cs.Field
			title: 'Middle Name(s)'
			message: 'Optional, unless you have middle names'
...
introViewPassword = new IntroViewPassword
...
homeView = new HomeView
	...
	steps: [
		...
		{title: 'Current Address', icon: 'home', addressView}
		{title: 'Documents', icon: 'file-multiple', docView}
	]
	step: 1
