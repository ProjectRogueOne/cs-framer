...
# 		@uploadIcon.icon = 'check'
	...
	showUploadFail: (response) =>
		...
		switch response 
			when 'filesize'
				@messageLayer.text = 'Oops! It looks like the file you uploaded is in the wrong format. Try uploading a .jpeg, .gif, .pdf or .png file.'
			when 'filetype'
...
class Advice extends Layer
	...
	setHeight: =>
		_.assign @,
			height: Math.max(@textLayer.height, @illo.height)
